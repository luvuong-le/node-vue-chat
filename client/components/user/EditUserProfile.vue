		...
		<div class="section profile__content">
            ...
            <form @submit.prevent="handleSubmit" class="form">
                ...
                <div class="form__input-group">
                    ...
                    <input type="text" name="username" class="form__control" placeholder="Enter New Username" v-model.trim='username'>
                    <label for="username" class="form__label">Username</label>
                </div>
                <div class="form__input-group">
                    <ion-icon name="person" class="form__icon"></ion-icon>
                    <input type="email" name="email" class="form__control" placeholder="Enter New Email" v-model.trim="email">
                    <label for="email" class="form__label">Email</label>
                </div>
                <div class="form__input-group">
                    <ion-icon name="map" class="form__icon"></ion-icon>
                    <input type="location" name="location" class="form__control" placeholder="Enter New Location" v-model.trim="location">
...
export default {
	...
	data: function() {
		return {
			user: {},
			email: this.$store.getters.getUserData.email || JSON.parse(localStorage.getItem('user')).email,
			username: this.$store.getters.getUserData.username || JSON.parse(localStorage.getItem('user')).username,
			location: this.$store.getters.getUserData.location || JSON.parse(localStorage.getItem('user')).location,
			errors: [],
		};
	},
	computed: {
	...
	methods: {
		...
		handleSubmit(e) {
			...
			if (localStorage.getItem('session_id')) {
				axios
					.put(`http://localhost:5000/api/user/${localStorage.getItem('session_id')}`, updatedUserDetails, {
						headers: {
